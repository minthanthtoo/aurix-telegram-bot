...
class TelegramCommandMixin:
    def _show_capacity(
        ...
        try:
            ...
            self.send(chat_id, "Outline capacity metrics are temporarily unavailable.")
        ...
        endpoint_lines = []
        endpoint_buttons = []
        metric_errors = snapshot.get("metrics_errors") or {}
        for endpoint in (snapshot.get("endpoints") or [])[:10]:
            endpoint_id = str(endpoint["id"])
            maximum = endpoint.get("max_active_keys")
            capacity = f"{endpoint['active_assignments']}/{maximum or '∞'}"
            gate = "open" if endpoint.get("accepts_new_assignments") else "paused"
            health = "⚠️" if endpoint_id in metric_errors else "✅"
            endpoint_lines.append(
                f"{health} {endpoint['code']} · {endpoint['state']} · {gate} · "
                f"slots {capacity} · mapped {self._format_bytes(endpoint.get('mapped_transfer_bytes', 0))}"
            )
            endpoint_buttons.append(
                [
                    (
                        "⏸ Pause " + str(endpoint["code"])
                        if endpoint.get("accepts_new_assignments")
                        else "▶️ Open " + str(endpoint["code"]),
                        f"a:e:t,{endpoint_id}",
                    ),
        ...
        markup = self._inline_keyboard(
            endpoint_buttons
            + [[("🔄 Refresh", "a:n:capacity"), ("🛠 Admin", "a:n:admin")]]
        )
    ...
    def _show_endpoint_plans(
        ...
        plans = self._admin_call(telegram_id, "endpoint_plan_capacity", endpoint_id)
        blocks = ["📋 Endpoint plan allocation", f"Endpoint: {endpoint_id}"]
        rows = []
        for plan in plans:
            code = str(plan["plan_code"])
            enabled = bool(plan["enabled"])
            maximum = plan.get("max_active_assignments")
            label = "blocked" if not enabled else str(maximum or "∞")
            blocks.append(
    ...
    def handle(self, message: dict[str, Any]) -> None:
        ...
        raw_text = text.strip()
        text = None
        pending_input = self._consume_customer_input(telegram_id, raw_text)
        if pending_input is not None:
            text = pending_input
        if text is None and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{2,31}", raw_text):
            ...
            if not self._is_admin(telegram_id):
                ...
                try:
                    if command == "/approve":
                        ...
                        outcome = (
                            "approved; wallet credit posted."
                            if result.status in ("wallet_credited", "already_credited")
                            else "approved; provisioning queued."
                        )
                        self.send(
                            chat["id"],
                            f"Order {result.order_id} {outcome}",
                            self._inline_keyboard(
                                [
                                    [
