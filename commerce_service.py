...
class CommerceService(CommerceWorkerMixin):
    ...
    def create_wallet_topup(
        ...
        """Create one receipt-backed wallet deposit request.

        Top-ups deliberately reuse the existing financial review state machine,
        but approval credits the wallet and never creates a VPN subscription.
        """
        try:
        ...
        if amount_minor < 1_000:
            raise CommerceError("Minimum wallet top-up is 1,000 MMK")
        if amount_minor > 1_000_000:
            raise CommerceError("Maximum wallet top-up is 1,000,000 MMK")
        ...
        with self.database.connect() as connection:
            ...
            if existing is not None:
                ...
                return OrderResult(
                    str(existing["id"]), existing_plan, str(existing["status"]), False,
            ...
            connection.execute(
                """INSERT INTO orders
                   ...
                   VALUES (?, ?, 'wallet_topup', ?, 'MMK', 'Wallet Top-up',
                           NULL, 1, 'awaiting_payment', ?, 'wallet_topup')""",
    ...
    def select_payment_provider(
        self, telegram_id: int, order_id: str, provider_code: str
    ) -> dict[str, Any]:
        code = str(provider_code or "").strip().lower()
        provider = self.PAYMENT_PROVIDERS.get(code)
        if provider is None:
        ...
        with self.database.connect() as connection:
            ...
            if order["status"] != "awaiting_payment":
                raise CommerceError("Payment method cannot be changed after receipt submission")
            connection.execute(
                "UPDATE orders SET selected_payment_provider = ? WHERE id = ?",
                (provider, order_id),
            )
            self._audit(
                connection,
            ...
            result = dict(order)
            result["selected_payment_provider"] = provider
            return result
    ...
    def _receipt_risk_flags(
        ...
        tx_id = str(extraction.get("transaction_id") or "").strip()
        if not tx_id:
            flags.add("missing_transaction_id")
        amount = extraction.get("amount_minor")
        if amount is None:
            flags.add("missing_amount")
        else:
            try:
                if int(amount) != int(order["amount_minor"]):
        ...
        selected = str(order["selected_payment_provider"] or "").strip()
        ...
        expected_recipient = str(extraction.get("expected_recipient") or "").strip()
        extracted_recipient = str(extraction.get("recipient") or "").strip()
        if expected_recipient:
            if not extracted_recipient:
                flags.add("missing_recipient")
            else:
                expected = _normalize_reference(expected_recipient)
                actual = _normalize_reference(extracted_recipient)
    ...
    def receipt_duplicate_status(
        ...
        """Cheap pre-LLM duplicate check; submit_receipt remains authoritative."""
    ...
    def submit_receipt(
        ...
        digest = hashlib.sha256(image_bytes).hexdigest()
        extraction = dict(extraction) if isinstance(extraction, dict) else {}
        submitted_time = (now or datetime.now(UTC)).astimezone(UTC)
        submitted_at = _now_text(submitted_time)
        ...
        with self.database.connect() as connection:
            ...
            if existing is not None:
                ...
                status = str(existing["extraction_status"] or "needs_review")
                ...
                duplicate = connection.execute(
                    """SELECT id, order_id FROM payment_evidence
                       WHERE order_id != ? AND image_sha256 = ? LIMIT 1""",
                    (order_id, digest),
                ).fetchone()
                if duplicate is None and file_unique_id:
                    duplicate = connection.execute(
                        """SELECT id, order_id FROM payment_evidence
                           WHERE order_id != ? AND telegram_file_unique_id = ? LIMIT 1""",
                        (order_id, str(file_unique_id)[:256]),
                    ).fetchone()
                if duplicate is not None:
                    self._audit(
                        ...
                        "customer",
                        str(telegram_id),
                        {"prior_evidence_id": str(duplicate["id"])},
                    )
                    raise CommerceError(
                ...
                selected_provider = str(order["selected_payment_provider"] or "").strip()
                extracted_provider = str(extraction.get("provider") or "").strip()
                provider_name = (selected_provider or extracted_provider or provider).strip()[:64]
                extraction["flags"] = self._receipt_risk_flags(
                    extraction, order, submitted_time
                )
                tx_id = extraction.get("transaction_id")
                tx_candidate = str(tx_id).strip()[:128] if tx_id else ""
                status = (
                    "parsed"
                    if tx_candidate and not extraction["flags"]
                    else "needs_review"
                )
    ...
    def verify_receipt(
        ...
        with self.database.connect() as connection:
            ...
            if (
                str(evidence["plan_code"] if "plan_code" in evidence.keys() else "")
                == "wallet_topup"
            ...
            try:
                ...
                connection.execute(
                    """INSERT INTO verified_payment_references
                       (provider_key, normalized_reference, payment_id, order_id, verified_at)
                       VALUES (?, ?, ?, ?, ?)""",
                    (
                        normalized_provider,
                        normalized_reference,
    ...
    def consistency_report(
        ...
        with self.database.connect() as connection:
            ...
            verified_reference_mismatches = connection.execute(
                """SELECT COUNT(*) AS n FROM payments p
                   WHERE p.status = 'verified' AND NOT EXISTS (
                       SELECT 1 FROM verified_payment_references r
            ...
            if is_wallet_topup:
                ...
                connection.execute(
                    ...
                    (
                        ...
                        order["telegram_id"],
                        f"✅ Your wallet top-up of {int(order['amount_minor']):,} "
                        f"{order['currency']} was approved.",
                        starts_at,
                        starts_at,
                    ),
                )
