...
This configuration is a durable single-process bot/control-plane MVP, not a
horizontally scaled bot deployment. Long polling, manual payment authority, and
at-least-once Telegram notification delivery remain deliberate limits. The code
is endpoint-aware, but the hosted release should begin with one bootstrap Outline
server and keep provider mutation/new-endpoint activation disabled until the
autoscale runbook's acceptance gates pass. Dead-lettered notifications and
terminal jobs are visible to admins for manual recovery.
...
and maintenance timings without request payloads or credentials. Each managed
Outline endpoint is observed independently; one endpoint failure does not reuse
another endpoint's key IDs or reset its durable usage. Return logging to
