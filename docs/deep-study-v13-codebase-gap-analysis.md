...
> **Historical snapshot notice (2026-08-29):** This study describes the pre-Phase-8 codebase and its line-level references, test counts, module sizes, and gap scores are no longer current. Use [`CURRENT_SNAPSHOT_TO_V13_PLAN.md`](CURRENT_SNAPSHOT_TO_V13_PLAN.md) for the re-audited current classification and implementation roadmap. This file is retained as historical reasoning, not as the current-state report.
