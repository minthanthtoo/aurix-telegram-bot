...
Status date: 2026-09-05 (Asia/Rangoon)
...
| Quota exhaustion enforcement | Implemented locally | Per-key and account-aggregate metrics `used >= configured limit` fail closed, record deduplicated termination/audit state, revoke every active generation for the exhausted entitlement, and queue hard DELETE; Outline has no documented pause endpoint |
| Usage/capacity operations | Implemented locally | Admin `/capacity`, stable transfer-metrics adapter, mapped active-key totals, server-scoped joins, bounded durable usage snapshots (default 5-minute cadence / 90-day retention), account-level monotonic usage epochs with counter-reset/deduplication protection across endpoints, conservative customer totals, and read-only per-node admission/policy posture (freshness, headroom, over-allocation, orphan audit); an unobserved endpoint leaves remaining balance explicitly unknown |
...
| Independent worker / web control plane | Worker and callback endpoint implemented; live enrollment pending | The guarded DigitalOcean worker/timer, encrypted one-time `/fleet/register` callback, authenticated server-agent probe API, signed managed-device API, and shared-database Telegram result projection are in source. The recovery/autodeploy paths install the probe API unit from the same CI-approved release and enable it only when `AURIX_PROBE_API_ENABLED=1` plus both signing/encryption keys are present; live endpoint/worker canary remains a gate. Render profiles run read-only Telegram, Supabase, LLM, database, and pinned-Outline startup canaries. |
| Live Telegram and Outline smoke test | Degraded for admission: 1/3 nodes currently carries customer keys | From the control-plane host, all three Outline 1.12.3 management APIs respond. Primary (`157.245.63.95`) is healthy with 17 remote keys, 8 metrics keys, and data port 45524 open. Singapore-B and BKK have zero remote keys; Outline normally has no data listener while a node is empty, so those nodes are healthy-but-not-serving. A reversible 1 MB create → data-port-open → delete canary passed on both empty nodes; they remain at zero saleable slots until the owner opens a tranche. The live release SHA is checked by `production_acceptance.py` at deploy time (the versioned status prose is not authoritative for that value); the bot and recovery timers are active with no recent bot errors. Customer views now withhold stale credentials during an open repair and queue a deduplicated user notice. A real customer-account canary and longer observation remain |
...
- Local test/evidence coverage: **100%** for the current fake-Outline, TLS,
  ...
  production-acceptance suite (494 tests passing at the latest verification).
- Live deployment readiness: **staged, not 100%**. The deployed control plane
  is validated by the live `production_acceptance.py` release-marker check,
  all required systemd services/timers are active, and the control-plane-host
  canary reports 3/3 management APIs reachable. Running
  ...
  private object store and enforces the 48-hour freshness budget (latest live
  verification: approximately 35 hours old). The overall result remains `warn` because the primary
  ...
  show only primary serving traffic; Singapore-B and BKK have no keys (and
  therefore no expected access listener) and must remain unsaleable until an
  owner-approved tranche is followed by a real key/data-plane canary. A real Telegram
