...
class CommerceWorkerMixin:
        ...
        for row in rows:
            ...
            try:
                ...
                if self.connectivity is not None:
                    gateway = self.connectivity.client(str(row["endpoint_id"]))
                gateway.delete_key(str(row["outline_key_id"]))
                with self.database.connect() as connection:
                    self.database.begin_write(connection)
                    ...
                    connection.execute(
                        ...
                        (
                            ...
                            row["data_limit_bytes"], row["expires_at"], timestamp, timestamp,
                        ),
                    )
    ...
    def _revoke(self, job: dict[str, Any], now: datetime) -> None:
        ...
        try:
            outline = self.outline
            connectivity = getattr(self, "connectivity", None)
            if connectivity is not None and key["endpoint_id"]:
                outline = connectivity.client(str(key["endpoint_id"]))
