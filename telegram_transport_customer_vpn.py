...
from telegram_customer_vpn_detail import observe_paid_key_detail, render_paid_key_detail
    ...
    def _send_paid_key_detail(
        ...
        *,
        ...
        if item is None:
            ...
            return
        state = observe_paid_key_detail(self, item)
        text, markup = render_paid_key_detail(
            state,
            subscription_id,
            format_bytes=self._format_bytes,
            copy_text_button=lambda value: self._copy_text_button("📋 Copy Outline Key", value),
        )
        if isinstance(message_id, int):
            self.edit_message(chat_id, message_id, text, markup)
        else:
