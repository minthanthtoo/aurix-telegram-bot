...
3. **Map design possibilities.** Read `references/type-systems-and-layout.md`, then select candidates across genuinely different roles (for example, a highly legible text sans, a distinctive display serif, a condensed numeric face, and a mono utility face). Treat category associations as hypotheses to test, not universal psychology. If the brief references TikTok, CapCut, Reels, or “more font variety,” run a market-style scan across at least six directions: clean/platform, condensed/impact, rounded/playful, handwritten/sticker, editorial serif, and mono/technical. Separate actual font families from app templates, generated text, outlines, stickers, and motion presets; an app library is a source of creative categories, not a brand kit.
4. **Render a controlled comparison.** Hold copy, dimensions, color field, and baseline constant while changing one variable at a time: family, weight, width, role contrast, numeral feature, color, shadow, outline, or underline. Include at least one real Burmese phrase when Myanmar script is involved, and pair every Latin display candidate with the same tested Burmese companion instead of assuming fallback coverage. Save a broad category board for exploration and a smaller role board for the actual recommendation; the broad board is evidence, not a license to use every font in one post.
...
- when variety is requested, one broad category/market-scan board (roughly 18–24 directions) plus a compact Burmese role board;
