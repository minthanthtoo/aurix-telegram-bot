...
class MvpFeatureTest(unittest.TestCase):
    ...
    def test_llm_prompt_preserves_zero_decimal_mmk_amounts(self):
        provider_response = io.StringIO(
            json.dumps(
                {
                    "choices": [
                        {
        ...
        with patch("receipt_llm.urllib.request.urlopen", return_value=provider_response) as opened:
            parsed = extractor.extract(b"synthetic image", "image/png")

        request_body = json.loads(opened.call_args.args[0].data)
        prompt = request_body["messages"][0]["content"]
        self.assertIn("MMK is zero-decimal", prompt)
        self.assertIn("12,500 MMK", prompt)
        self.assertIn("amount_minor 12500", prompt)
        self.assertEqual(parsed.amount_minor, 12500)
