...
def _required_environment(settings: Mapping[str, str]) -> tuple[str, str]:
    ...
    return token, access_url_key


def _outline_environment(
    ...
    return api_url, fingerprint, servers_json, outline_timeout, outline_cooldown


def _service_environment(
    settings: Mapping[str, str],
) -> tuple[str, bool, str, str, str, tuple[str, str, str], str]:
    ...
    return (
        ...
        receipt_storage_required,
        supabase_url,
        supabase_service_key,
        settings.get("SUPABASE_RECEIPTS_BUCKET", "payment-receipts"),
        receipt_llm_config,
...
def _probe_environment(
    settings: Mapping[str, str],
) -> tuple[dict[str, str], int, int, list[dict[str, Any]], list[dict[str, Any]]]:
    try:
...
def _access_environment(settings: Mapping[str, str]) -> tuple[set[int], int | None, int | None, set[int], set[int]]:
    ...
    return (
        _parse_ids(settings, "ADMIN_TELEGRAM_IDS"),
        next(iter(owner_ids), None),
        control_group_id,
        _parse_ids(settings, "ADMIN_SCOPE_CLEANUP_IDS"),
        _parse_ids(settings, "TRIAL_TELEGRAM_IDS"),
    )


def _maintenance_interval(settings: Mapping[str, str]) -> float:
    try:
        value = float(
            settings.get(
...
class RuntimeSettings:
    ...
    def from_environment(cls, env: Mapping[str, str] | None = None) -> RuntimeSettings:
        ...
        token, access_url_key = _required_environment(settings)
        api_url, fingerprint, servers_json, outline_timeout, outline_cooldown = _outline_environment(
            settings
        )
        (
            commerce_database_url,
        ...
        (
            ...
            configured_targets,
            configured_schedules,
        ) = _probe_environment(settings)
        admin_ids, owner_id, control_group_id, cleanup_ids, trial_ids = _access_environment(
            settings
        )
        return cls(
            token=token,
            ...
            supabase_receipts_bucket=supabase_receipts_bucket,
            ...
            in ("1", "true", "yes"),
            probe_agent_secrets=probe_agent_secrets,
            ...
            command_scope_cleanup_ids=cleanup_ids,
            trial_ids=trial_ids,
            maintenance_interval_seconds=_maintenance_interval(settings),
