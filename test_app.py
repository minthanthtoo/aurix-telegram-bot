...
class ClaimServiceTest(unittest.TestCase):
    ...
    def test_unavailable_endpoint_metrics_do_not_reset_last_free_usage(self):
        self.service.claim(123, "Min", self.now)
        self.service.enforce_quota(
            self.now + timedelta(hours=1),
            metrics={"byEndpoint": {"legacy-default": {"1": 12345}}, "errors": {}},
        )
        self.service.enforce_quota(
        ...
        with self.db.connect() as connection:
            observed = connection.execute(
                "SELECT last_usage_bytes FROM keys WHERE outline_key_id = '1'"
    ...
    def test_outline_key_identity_is_endpoint_scoped(self):
        with open_sqlite_connection(self.db.path) as conn:
            indexes = conn.execute("PRAGMA index_list(keys)").fetchall()
            composite = next(idx for idx in indexes if idx[1] == "keys_endpoint_external")
            columns = [row[2] for row in conn.execute("PRAGMA index_info(keys_endpoint_external)")]
        self.assertEqual(composite[2], 1)
        self.assertEqual(columns, ["endpoint_id", "outline_key_id"])
...
class TelegramBotCommerceTest(unittest.TestCase):
    ...
    def test_repeated_buy_returns_existing_order_and_myorders_tracks_it(self):
        ...
        self.assertIn("🧾 My Orders", self.bot.sent[-1][1])
        self.assertIn(order_id[:10], self.bot.sent[-1][1])
    ...
    def test_myorders_filters_and_pages_edit_the_pressed_message(self):
        first = self.commerce.create_order(123, "Min", "basic_50gb")
        self.commerce.cancel_order(123, first.order_id)
        self.commerce.create_order(123, "Min", "standard_100gb")
        self.bot.handle(self.message(123, "/myorders"))
        cancelled = next(
            button["callback_data"]
            for row in self.bot.markups[-1]["inline_keyboard"]
            for button in row
            if button["text"].startswith("🚫 Cancelled")
        )
        calls = []
        ...
        self.bot.handle_callback(
            {
                ...
                "from": {"id": 123, "first_name": "Min"},
                "message": {"chat": {"id": 123, "type": "private"}, "message_id": 81},
                "data": cancelled,
            }
        )
    ...
    def test_wallet_topup_amount_and_provider_flow_uses_buttons(self):
        ...
        order = self.commerce.list_user_orders(123, limit=1)[0]
        self.assertEqual(order["order_type"], "wallet_topup")
        self.assertEqual(order["amount_minor"], 6000)
    ...
    def test_button_actions_and_whoami(self):
        ...
        self.assertIn("Nothing in this category", self.bot.sent[-1][1])
        self.bot.handle(self.message(123, "/whoami"))
        ...
        self.bot.handle_callback(
            {
                "id": "vpn-refresh",
                "from": {"id": 123, "first_name": "Min"},
                "message": {"chat": {"id": 123, "type": "private"}, "message_id": 91},
                "data": refresh,
            }
        )
        self.assertEqual(len(self.bot.sent), sent_before)
        self.assertTrue(any(method == "editMessageText" for method, _payload in calls))
    ...
    def test_myvpn_status_and_usage_aliases_converge_on_one_dashboard(self):
        ...
        self.assertIn("Active · 1 key(s)", dashboards[0])

    def test_myvpn_does_not_duplicate_order_navigation(self):
        order = self.commerce.create_order(123, "Min", "basic_50gb")

        self.bot.handle(self.message(123, "/myvpn"))

        self.assertNotIn(f"Open order {order.order_id[:8]}", self.bot.sent[-1][1])
        buttons = [
            button for row in self.bot.markups[-1]["inline_keyboard"] for button in row
        ]
        self.assertFalse(
    ...
    def test_paid_key_is_retrievable_and_copyable_from_myvpn(self):
        ...
        self.assertFalse(any(button["text"].startswith("🔄 Renew") for button in buttons))
