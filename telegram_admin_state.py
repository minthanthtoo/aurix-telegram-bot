...
def _preview_receipt_mode(snapshot: dict[str, Any]) -> str:
    return "\n".join(
        [
...
def _preview_promo(command: str, args: list[str], snapshot: dict[str, Any]) -> str:
...
def _preview_server_state(command: str, args: list[str], snapshot: dict[str, Any]) -> str:
    requested = str(
        snapshot.get("requested_state") or args[1] if len(args) > 1 else "-"
    ).lower()
    lines = [
...
def _preview_migration(args: list[str], snapshot: dict[str, Any]) -> str:
    return "\n".join(
        [
...
def _preview_repair(args: list[str], snapshot: dict[str, Any]) -> str:
...
def _preview_retry_job(args: list[str], snapshot: dict[str, Any]) -> str:
    return "\n".join(
        [
...
def _preview_receipt(command: str, args: list[str], snapshot: dict[str, Any]) -> str:
...
def _preview_order(command: str, args: list[str], snapshot: dict[str, Any]) -> str:
...
class TelegramAdminStateMixin(TelegramAdminSnapshotMixin):
    ...
    def _admin_preview_text(
        ...
        if command == "/receiptmode":
            return _preview_receipt_mode(snapshot)
        if command in {"/setpromo", "/stoppromo", "/resumepromo"}:
            return _preview_promo(command, args, snapshot)
        if command == "/serverstate":
            return _preview_server_state(command, args, snapshot)
        if command == "/migratekey":
            return _preview_migration(args, snapshot)
        if command == "/approverepair":
            return _preview_repair(args, snapshot)
        if command == "/retryjob":
            return _preview_retry_job(args, snapshot)
        if command in {"/verify", "/rejectreceipt"}:
            return _preview_receipt(command, args, snapshot)
        return _preview_order(command, args, snapshot)
