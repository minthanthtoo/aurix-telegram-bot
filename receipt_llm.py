...
class OpenAICompatibleReceiptExtractor:
    ...
    def extract(self, image_bytes: bytes, mime_type: str = "image/jpeg") -> ReceiptExtraction:
        ...
        amount_guidance = (
            "amount_minor is the integer amount in the currency's real minor unit; "
            "never assume every currency has two decimal places. MMK is zero-decimal, "
            "so a receipt showing 12,500 MMK must return amount_minor 12500, not 1250000. "
        )
        body = {
            "model": self.model,
            "temperature": 0,
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        f"exactly these fields: {json.dumps(schema_hint)}. {amount_guidance}"
                        "Recognize KBZPay, WavePay, AYA Pay, uabpay, and CB Pay receipts. "
                        "transaction_id must be the provider's unique transfer/reference ID, not "
                        "a phone number, account number, QR payload, amount, or date. Return the "
                        "visible transaction time as ISO-8601 including timezone when shown. "
                        "Add flags for failed/pending/reversed status, edited-looking regions, "
                        "conflicting amounts, missing recipient, or ambiguous identifiers."
                    ),
                },
                {
                    "role": "user",
