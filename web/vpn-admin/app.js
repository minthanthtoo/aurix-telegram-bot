(function () {
  ...
  function renderEndpointDetail(data) {
    ...
    const credentials = (data.credentials || []).map((c) => {
      const phase = c.lifecycle_phase || c.status || "unknown";
      const phaseTone = phase === "active" || phase === "revoked" ? "good" : "warn";
      return `<tr><td><strong>${esc(c.protocol)}</strong><small>${esc(c.generation_id)}</small></td><td>${esc(c.entitlement_key)}</td><td>${badge(c.status, c.status === "active" ? "good" : "neutral")}</td><td>${badge(c.remote_state, c.remote_state === "observed" ? "good" : "warn")}</td><td>${badge(phase, phaseTone)}</td><td>${c.quota_bytes == null ? "—" : `${fmt(c.consumed_bytes)} / ${fmt(c.quota_bytes)}`}</td><td>${fmt(c.active_lease_bytes)} / ${fmt(c.lease_used_bytes)}</td><td>${esc(c.created_at)}</td></tr>`;
    });
  ...
  function renderAccountDetail(data) {
    ...
    const enforcement = account.access_enforcement || {};
    const enforcementCounts = enforcement.counts || {};
    const enforcementState = enforcement.account_status === "suspended"
      ? (enforcement.ready_to_reactivate ? "ready" : "in progress")
      : enforcement.account_status || "unknown";
    const enforcementTone = enforcementState === "active" || enforcementState === "ready" ? "good" : "warn";
    ...
    $("account-detail").innerHTML = `<article class="panel"><div class="panel-head"><div><p class="eyebrow">ACCOUNT DETAIL</p><h2>${esc(account.account_id)}</h2><p class="muted">Telegram ${esc(account.telegram_id)} · ${esc(account.status)}</p></div><span class="read-only">NO SECRETS</span></div><div class="metrics mini">${card("Subscriptions", fmt((account.subscriptions || []).length), "durable entitlements")}${card("Devices", fmt((account.devices || []).length), "enrolled records")}${card("Routes", fmt((account.routes || []).length), "active observed paths")}${card("Revocation epoch", fmt(account.revocation_epoch), "account-wide control")}</div><div class="detail-block"><div class="panel-head"><div><h3>Access enforcement</h3><p class="muted">Read-only revoke and reactivation proof; operator mutations remain in Telegram.</p></div>${badge(enforcementState, enforcementTone)}</div><div class="check-grid"><div><span>Account state</span><strong>${esc(enforcement.account_status || account.status || "—")}</strong></div><div><span>Action</span><strong>${esc(enforcement.target_status || "—")} · ${esc(enforcement.state || "—")}</strong></div><div><span>Outstanding</span><strong>${fmt(enforcement.outstanding)}</strong></div><div><span>Paid / free keys</span><strong>${fmt(enforcementCounts.active_paid_keys)} / ${fmt(enforcementCounts.active_free_keys)}</strong></div><div><span>Revoke jobs</span><strong>${fmt(enforcementCounts.pending_revoke_jobs)}</strong></div><div><span>Credential generations</span><strong>${fmt(enforcementCounts.active_generations)}</strong></div></div><p class="muted">${enforcement.ready_to_reactivate ? "Remote revoke and session proof are complete; reactivation is eligible." : enforcement.account_status === "suspended" ? "Reactivation remains blocked until all remote revoke and session proof is complete." : "No account-level enforcement is pending."}</p></div><div class="detail-block"><h3>Subscriptions / entitlements</h3>${table(["Plan", "State", "Endpoint", "Credential", "Expires"], subscriptions)}</div><div class="grid two"><div><h3>Devices</h3>${table(["Device", "State", "Last seen", "Revoked"], devices)}</div><div><h3>Active routes</h3>${table(["Protocol / route", "Endpoint", "Region", "Generation"], routes)}</div></div></article>`;
  }
