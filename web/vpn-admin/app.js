(function () {
  ...
  function renderEndpointDetail(data) {
    ...
    $("endpoint-detail").innerHTML = `<article class="panel"><div class="panel-head"><div><p class="eyebrow">NODE DETAIL</p><h2>${esc(endpoint.code || endpoint.id)}</h2><p class="muted">${esc(endpoint.provider)} · ${esc(endpoint.region)} · ${esc(endpoint.state)}</p></div><span class="read-only">NO SECRETS</span></div><div class="metrics mini">${card("Health", endpoint.healthy ? "healthy" : "stale", endpoint.last_healthy_at ? `last ${endpoint.last_healthy_at}` : "no fresh observation", endpoint.healthy ? "good" : "warn")}${card("Active assignments", fmt(endpoint.active_assignments), endpoint.max_active_keys ? `of ${fmt(endpoint.max_active_keys)}` : "no hard cap")}${card("Credentials", fmt(credentials.length), "generation records")}${card("Allocation", endpoint.accepts_new_assignments ? "open" : "paused", "new assignments", endpoint.accepts_new_assignments ? "good" : "warn")}</div><div class="detail-block"><h3>Capacity by plan</h3>${table(["Plan", "Policy", "Active assignments"], capacity)}</div><div class="grid two"><div><h3>Assignments</h3>${table(["Plan", "Protocol", "Entitlement", "State", "Reserved bytes", "Reason"], assignments)}</div><div><h3>Credential generations</h3>${table(["Protocol", "Entitlement", "State", "Remote", "Created"], credentials)}</div></div><div class="detail-block"><h3>Protocol profiles</h3>${table(["Protocol / adapter", "State", "Capabilities", "Verified", "Last healthy"], protocols)}</div><div class="detail-block"><h3>Recent protocol evidence</h3>${table(["Protocol / signal", "Status", "Details", "Source", "Observed"], observations)}</div></article>`;
  }
