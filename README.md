...
Neither profile is safe to scale the bot process beyond one instance. The data
model and workers are endpoint-aware, but provider-driven VPN fleet expansion
remains mutation-disabled until its rollout and evidence gates pass. Independent
bot workers, automated payment-provider verification, referrals, affiliates,
and resellers remain gated. See
...
- `PAYMENT_QR_KPAY`, `PAYMENT_QR_WAVEPAY`, `PAYMENT_QR_AYAPAY`, `PAYMENT_QR_UABPAY`, `PAYMENT_QR_CBPAY` — Telegram photo `file_id` or stable HTTPS URL for the corresponding payment QR. Never use a receipt screenshot here.
...
| `PAYMENT_QR_*` / `PAYMENT_RECIPIENT_*` | Configure all payment methods you expose; missing QR values fail closed and tell customers not to transfer |
