...
Wallet events are immutable. A wallet top-up accepts an exact receipt amount only, credits the balance exactly once after human verification, and never provisions a VPN subscription. A subsequent wallet purchase records `reserve → capture`; rejection releases a reservation exactly once. Capture does not deduct the balance a second time. Receipt SHA-256 and Telegram file identity are rejected across different orders before vision processing; a low-threshold perceptual fingerprint now flags resized/re-encoded near-duplicates for manual review without hard-rejecting legitimate provider templates. Normalized provider transaction IDs remain the authoritative duplicate check at verification, and the database enforces case-insensitive provider/reference uniqueness atomically (legacy collisions stop startup for manual reconciliation). AI triage fails closed when a merchant profile is missing and always leaves financial approval to staff. `/wallet` shows the current projection and recent ledger events, while `/reconcile` reports balance mismatches and impossible order/job combinations.
