...
class CommerceDatabase:
    ...
    def initialize(self) -> None:
        ...
        with self.connect() as connection:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS verified_payment_references (
                    provider_key TEXT NOT NULL,
                    normalized_reference TEXT NOT NULL,
                    payment_id TEXT NOT NULL REFERENCES payments(id),
                    order_id TEXT NOT NULL REFERENCES orders(id),
                    verified_at TEXT NOT NULL,
                    PRIMARY KEY(provider_key, normalized_reference)
                );
            ...
            if "normalized_reference" not in payment_columns:
                ...
                payment_columns.add("normalized_reference")
            ...
            if {
                ...
                connection.execute(
                    """INSERT INTO verified_payment_references
                       ...
                       SELECT LOWER(REPLACE(provider, ' ', '')), normalized_reference,
                              id, order_id, COALESCE(verified_at, submitted_at)
                       FROM payments WHERE status = 'verified' AND normalized_reference != ''
                       ON CONFLICT(provider_key, normalized_reference) DO NOTHING"""
                )
            ...
            for name, definition in (
                ...
                ("duration_days_snapshot", "INTEGER"),
                ("refund_status", "TEXT NOT NULL DEFAULT 'none'"),
                ("order_type", "TEXT NOT NULL DEFAULT 'vpn'"),
                ("selected_payment_provider", "TEXT"),
            ):
    ...
    def _seed_plans(connection: Any) -> None:
        ...
        connection.execute(
            "UPDATE plans SET name = 'Wallet Top-up', price_minor = 0, currency = 'MMK', quota_bytes = NULL, duration_days = 1, active = 0 WHERE code = 'wallet_topup'"
        )
...
class PostgresCommerceDatabase:
    ...
    def initialize(self) -> None:
        ...
        with self.connect() as connection:
            ...
            connection.execute(
                "ALTER TABLE orders ADD COLUMN IF NOT EXISTS order_type TEXT NOT NULL DEFAULT 'vpn'"
            )
            connection.execute(
