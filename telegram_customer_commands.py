...
def dispatch_onboarding(host: Any, context: TelegramCommandContext) -> bool:
    ...
    if command in ("/start", "/help"):
        ...
        if command == "/help":
            ...
            reply_markup = host._customer_keyboard(telegram_id)
        host.send(chat_id, welcome_text, reply_markup)
        return True

    if command == "/whoami":
        access = "\nAdmin access: enabled" if host._is_admin(telegram_id) else ""
        host.send(
            chat_id,
    ...
    promo_code = context.args[0] if command == "/claimpromo" and context.args else None
    try:
    ...
    if result.outcome == "won":
        ...
        host.send(chat_id, f"This account is not eligible: {result.reason}")
    elif result.outcome == "scheduled":
        host.send(chat_id, "This promo has not started yet. Open Plans later to refresh.")
        ...
        host.send(chat_id, "This promo's current giveaway window is fully claimed.")
    return True
